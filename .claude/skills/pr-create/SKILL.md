---
name: pr-create
description: Creates a pull request from current changes, monitors GitHub CI, debugs any failures until CI passes, then monitors for Copilot review comments and addresses them. Use this when the user says "create pr", "make a pr", "open pull request", "submit pr", "pr for these changes", or wants to get their current work into a reviewable PR. Assumes the project uses git, is hosted on GitHub, and has GitHub Actions CI with automated checks (lint, build, tests, etc.). Does NOT merge - stops when CI passes and Copilot review is addressed.
...
Get changes into a PR, monitor CI, fix any failures, wait for Copilot review, address review comments, and notify the user when the PR is ready for human review.
...
[Main Task] "Create PR: [branch-name]"
  └── [CI Task] "CI Run #1" (status: failed, reason: lint)
      └── [Fix Task] "Fix: lint"
  └── [CI Task] "CI Run #2" (status: passed)
  └── [Review Task] "Copilot Review: waiting"
      └── [Fix Task] "Fix: review comment about X"
  └── [CI Task] "CI Run #3" (status: passed, post-review)
  └── [Review Task] "Copilot Review: resolved"
```
...
```
TaskCreate:
- subject: "Copilot Review: waiting"
- description: "Waiting for Copilot code review on PR #[number]"
- activeForm: "Waiting for Copilot review"
...
Copilot review typically takes 30-120 seconds after the PR is published. Poll with increasing intervals:

```bash
# Check for reviews on the PR
gh api repos/{owner}/{repo}/pulls/{number}/reviews --jq '.[] | "\(.id) \(.user.login) \(.state)"'

# Check for review comments (inline comments)
gh api repos/{owner}/{repo}/pulls/{number}/comments --jq '.[] | "\(.id) \(.user.login) \(.path):\(.line) \(.body)"'
```
...
**Identify Copilot reviews** by looking for reviews/comments from users like `copilot` (login contains "copilot" or the user is the GitHub Copilot bot).

### Step 12: Address Copilot Review Comments

Once Copilot review comments are found:

**Update the review task:**
```
TaskUpdate:
- taskId: [review task ID]
- subject: "Copilot Review: [N] comments"
- metadata: {"commentCount": N}
```

**Read each comment carefully:**

```bash
# Get full review comments with file context and node_id (needed for resolving)
gh api repos/{owner}/{repo}/pulls/{number}/comments --jq '.[] | select(.user.login | test("copilot"; "i")) | "### \(.path):\(.line)\nID: \(.id)\nNode ID: \(.node_id)\n\(.body)\n"'
```

**For each comment, determine the appropriate action:**

1. **Valid code improvement** — fix the code:
   - Read the relevant file and understand the context
   - Make the suggested change (or an equivalent fix that addresses the concern)
   - Create a fix task for tracking

2. **Style/preference suggestion that conflicts with project conventions** — dismiss with explanation:
   ```bash
   # Reply to the comment explaining why it's not applicable
   gh api repos/{owner}/{repo}/pulls/{number}/comments/{comment_id}/replies \
     -f body="This follows the project's existing convention. See [reference]."
   ```

3. **False positive or inapplicable suggestion** — dismiss with explanation:
   ```bash
   gh api repos/{owner}/{repo}/pulls/{number}/comments/{comment_id}/replies \
     -f body="[Brief explanation of why this doesn't apply]"
   ```

**Use `AskUserQuestion` for ambiguous comments** where the right action isn't clear. Present the comment and offer options.

**After addressing all comments, resolve the conversation threads** using the GraphQL API.

First, fetch the review thread IDs (these are `PRRT_` IDs, distinct from comment `PRRC_` IDs):
...
}' --jq '.data.repository.pullRequest.reviewThreads.nodes[] | select(.isResolved == false) | select(.comments.nodes[0].author.login | test("copilot"; "i")) | .id'
```

Then resolve each thread. **IMPORTANT:** Do NOT use shell variable substitution inside GraphQL queries — the `{` braces conflict with GraphQL syntax. Instead, hardcode each thread ID directly:

```bash
# List thread IDs first, then resolve each one individually.
# Copy-paste each PRRT_ ID into the command:
gh api graphql -f query="mutation { resolveReviewThread(input: {threadId: \"PRRT_xxxxx\"}) { thread { isResolved } } }" --jq '.data.resolveReviewThread.thread.isResolved'
```

When resolving multiple threads, run them in a loop by listing the IDs explicitly:
...
Before finalizing, ensure the branch is not behind the base branch:
...
**If up to date** (no output), proceed to Step 15.

**If behind**, rebase onto the base branch:

```bash
git rebase origin/<base-branch>
```

If conflicts arise, resolve them and continue the rebase. After resolving:
...
**Mark review and main PR tasks as completed:**
```
TaskUpdate:
- taskId: [review task ID]
- subject: "Copilot Review: resolved"
- status: "completed"
...
Call `TaskList` to gather all CI run, review, and fix tasks, then generate the summary:
...
**Copilot Review:** Addressed ([N] comments resolved)
**Reviewer:** @<handle> (if requested)

### CI Runs
- Run #1: Failed (lint) -> Fixed in [hash]
- Run #2: Passed
- Run #3: Passed (after review fixes)

### Copilot Review
- [file:line] — Fixed: [brief description]
- [file:line] — Dismissed: [brief reason]
...
│   └── Ensure branch is up to date (Step 14), publish (Step 15), final report (Step 16)
├── PR task completed
