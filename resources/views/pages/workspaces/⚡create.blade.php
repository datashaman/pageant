...
new #[Title('Create Workspace')] class extends Component {
    public string $name = '';
    ...
    public function save(): void
    {
        ...
        $workspace->agents()->sync($this->selectedAgents);
        $workspace->skills()->sync($this->selectedSkills);

        $this->redirect(route('workspaces.show', $workspace), navigate: true);
    }
}; ?>

<div class="w-full" data-chat-context="{{ json_encode(['page' => 'workspaces.create']) }}">
    <div class="space-y-6">
        <div class="flex items-center gap-4">
            <flux:button href="{{ route('workspaces.index') }}" wire:navigate>
                {{ __('Back') }}
            </flux:button>
        ...
        <form wire:submit="save" class="max-w-xl space-y-6">
            ...
            <x-form-actions :submit-label="__('Create')" :cancel-url="route('workspaces.index')" />
        </form>
    </div>
</div>
